...
for i = 1 : 10
    ...
    Coordinates_Faces(:, i) = xlsread(Filename_Excel, 1, xlRange(i, :));

end

save('Names_Faces.mat', 'Names_Faces');
save('Coordinates_Faces.mat', 'Coordinates_Faces');

%% Load Matrix Files From Excel

load('Names_Faces.mat');
load('Coordinates_Faces.mat');
...
for i1 = 1:size(Coordinates_Faces, 1)
    
    for i2 = 1 : 5
        F(i2, 1, i1) = Coordinates_Faces(i1, i2 * 2 - 1);
        F(i2, 2, i1) = Coordinates_Faces(i1, i2 * 2);
    end
...
while (Ind <= size(Names_Faces, 1))
    ...
    if (strcmp(Ind_Name,Ind_Name_Bis) == 0)
        ...
        for i = 1 : 3
            ...
            Names_Train_Images = [Names_Train_Images, Names_Faces(Ind)];
            Ind = Ind + 1;
        end
        ...
        Names_Test_Images = [Names_Test_Images, Names_Faces(Ind)];
        Ind = Ind + 1;
        Ind_Name_Bis = Ind_Name;
